<template>
	<view @touchmove="touchmove">
		...
		<view class="container" :style="{background: backColor}">
			...
			<view class="content"
				:style="{width: (pageNum * 100) + '%', height: windowHeight, WebkitColumnCount: pageNum,columnCount: pageNum,left: leftValue + 'px',fontSize: fontSize + 'rpx',lineHeight: lineHeight + 'px',color: fontColor}"
...
<script>
	import { getBookInfo, getChapters, cache, getCache } from '../../api/book';
	var isMoving = 0;
	...
	export default {
		...
		methods: {
			...
			onLoad(option) {
				...
				uni.getSystemInfo({
					success: (res) => {
						...
						let platform = res.platform; // 判断平台
						let navigationBarHeight = 0;
				
						if (platform === 'android') {
							navigationBarHeight = 48; // Android导航栏高度
						} else if (platform === 'ios') {
							navigationBarHeight = 44; // iOS导航栏高度
						} else {
							navigationBarHeight = 44; // devtools
						}
						that.windowHeight = (windowHeight - statusBarHeight - navigationBarHeight - 20) + 'px';
				...
				that.chapterIndex = redirectObj.chapterIndex;
			...
			refreshCatelog: function() {
				...
				var that = this;
				getChapters({sourceId: that.sourceId, link: that.bookLink}).then(res => {
					that.chapterList = res.data.chapterList;
					// 获取第一章链接
					that.initChapters(false, true);
				});
			},
			getScreenBrightness() {
			...
			initChapters: function(isLeftChapter, loadPageIndex) {
				var that = this;
				
				that.chapterName = that.chapterList[that.chapterIndex].name;
				
				uni.showLoading({
					title: '客官请稍后',
					mark: true
				});
				let link  = that.chapterList[that.chapterIndex].link;
				getBookInfo({sourceId: that.sourceId, link: link}).then(res => {
					...
					let pageNum = that.countPageNum(content, that.fontSize, that.lineHeight,
						that.windows.windowsWidth, that.windows.windowsHeight, that.windows.pixelRatio);
					
					// 读取用户设置 获取缓存内容
					getCache({
						openid: uni.getStorageSync('openid'),
						key: "pageindex_" + that.sourceId + "_" + that.bookName
					}).then(res => {
				...
				that.setReadProgress(that.chapterIndex);
			},
			...
			setPageIndex() {
				...
				cache({
					openid: uni.getStorageSync('openid'),
					key: "pageindex_" + this.sourceId + "_" + this.bookName,
					info: this.pageIndex
				});
			},
			// 触摸接触调用的函数
				...
				if (isMoving == 0) {
					...
					if (that.moveDirection == 0) {
						...
						if (currentIndex < that.pageNum) {
							...
							that.chapterIndex = that.chapterIndex + 1;
							that.initChapters(false, false);
						}
					} else {
