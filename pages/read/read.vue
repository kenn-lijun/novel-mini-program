<template>
	<view @touchmove="touchmove">
		...
		<view class="container" :style="{background: backColor, height: windowHeight}">
			<view class="content">
				<view>
					...
					<button class="leftButton" type="default" size="mini" @click="leftChapter" v-if="loaded">上一章</button>
					<button class="rightButton" type="default" size="mini" @click="rightChapter" v-if="loaded">下一章</button>
...
<script>
	import { getBookInfo, getChapters, cache } from '../../api/book';
	export default {
		data() {
			return {
				loaded: false,
			...
			onLoad(option) {
				...
				uni.getSystemInfo({
					success: (res) => {
						...
						that.windowHeight = (windowHeight - statusBarHeight - navigationBarHeight) + 'px';
					}
				})
				// 初始化页面配置
